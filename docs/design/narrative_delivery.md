...
기본 생존과 직업 정보를 선택적 랜덤 발견물에만 의존시키지 않는다. 초기 탐험 경험과 튜토리얼 수색꾼의 짧은 조언으로 다음 내용을 전달할 수 있어야 한다.

- 폐품과 잔재의 차이
- 무게 대비 가치 판단
- 코어가 파괴된 던전도 안전하지 않다는 점
- 감정 전 물품의 기본 처리
- 너무 깊이 들어가지 말아야 한다는 경고
- 퇴각 시점과 욕심의 위험성
...
| 정보 층 | 전달 원칙 |
|---|---|
| 생계와 직업 | 기본 회수·정산 흐름을 이해하도록 초기 플레이와 기능형 NPC를 통해 우선 전달 |
| 폐던전 생존 규칙 | 반복 탐험 전에 직접 경험과 튜토리얼 경고로 전달 |
| 세계 구조 | 발견물 등록과 연구자 해설에 따라 점진적으로 보강 |
| 마왕·봉인·부활 세력 | 선택적 발견물과 관련 NPC 반응으로만 제공 |
| 개별 서브 스토리 | 메인 생존 진행을 막지 않는 연속 단서로 제공 |

선택 정보가 없어도 기본 탐험과 생계 루프를 이해할 수 있어야 한다. 반대로 선택 정보를 모으면 새로운 전투 의무가 아니라 세계에 대한 해석과 수집 진행이 깊어진다.

## 연속 발견물과 지도 단서

실패한 수색꾼의 같은 표식, 필체, 장비와 지도 메모는 여러 탐험에 걸쳐 연결할 수 있다.

- 각 단서는 단독으로도 짧은 위험 경고나 분위기를 전달한다.
- 연결된 단서는 같은 인물이나 이동 경로를 추정하게 한다.
- 공식 지도 바깥의 개인 표식과 방향 메모는 잔여 구역을 암시하는 부분 단서로 사용한다.
- 지도 발견물은 정확한 정답지나 완성 지도를 즉시 제공하지 않는다.
