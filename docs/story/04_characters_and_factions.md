...
감정사 [`NPC-004`](../reference/entity_index.md)는 큰 임무를 가진 핵심 인물이 아니라, 거점에서 미확인 물품의 정체와 가치를 확인해 주는 기능형 NPC로 둔다. 현장 감정 수단이 있더라도 감정사는 더 공식적이고 확정적인 판별을 맡는다.

감정 상태, 거점·현장 방식의 제약과 처리 절차는 [`../design/item_rules.md#감정-상태`](../design/item_rules.md#감정-상태), 비용은 [`../design/economy_rules.md#비용-항목`](../design/economy_rules.md#비용-항목)이 상세 기준을 소유한다.

이 역할은 감정사를 스토리의 중심으로 만들지 않으면서도 미확인 물품의 불확실성과 자연스럽게 연결한다.
