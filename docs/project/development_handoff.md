...
- [x] `DEV-0110 — 하네스 분석과 비상 방전`: GDD에 확정된 최소 분석과 짧은 비상 무력화를 기존 충전 자원에 연결한다. `draft`인 Harness Engineering의 모듈 성장·최종 수치를 추정하지 않는다.
- [x] `DEV-0111 — G1 통합 플레이테스트와 수치 검토`: 보완한 조사·미확인 물품·골렘·하네스 행동을 작은 테스트 맵에 조립하고 GDD 20-1 및 `Q-004`·`Q-005` 판단 자료를 남겼다. 실제 사람 검토와 G1 통과 판단은 대기하며 제품 수치는 확정하지 않았다.
...
`DEV-0109-R1`은 구현·자동 검증을 완료하고 사용자 승인 후 `4f09ae6`으로 커밋했다. 이번 보완은 사람 플레이테스트나 `Q-004`·`Q-005`의 제품 수치 결정을 대신하지 않는다.

## DEV-0110 — 하네스 분석과 비상 방전

GDD 7-2·7-4·7-5와 9-2의 확정 범위에 따라 기존 잔해·경비 골렘에 분석과 짧은 정지를 연결했다. 상세 규칙이 비어 있는 `draft` Harness Engineering은 단독 구현 근거로 사용하지 않았다.

- 기본 분석은 가까운 대상의 징후·상태를 무료로 표시한다. 정밀 분석은 버튼에 표시된 충전을 써서 현재 상태와 대응 방법을 확인한다. 같은 대상의 같은 상태를 연속 구매하지 못하며, 대상·상태 변경과 범위 이탈 시 이전 읽기를 지운다.
- `Q`는 안내에 나온 잔해 안정화 또는 골렘 비상 방전을 실행한다. 감지 범위 안에서 대응 가능한 대상을 먼저 고르고 같은 조건에서는 가까운 대상을 고른다. 정밀 분석·안정화·방전은 하나의 충전을 함께 사용한다.
- 비상 방전은 이동과 의심·수색 타이머를 멈춘다. 정지 중 재방전·소음·경보·플레이어 재진입으로 정지를 취소하거나 늘리지 않는다. 복귀 시 현재 탐지 중인 플레이어가 있으면 경고 뒤 추적하고, 없으면 방전 전 마지막 위치를 수색한 뒤 순찰한다. 영구 처치는 허용하지 않는다.
- 충전 부족·잘못된 비용·사용 불가·삭제된 대상은 충전을 쓰지 않는다. 대상의 응답 전에 충전을 확보해 중첩 명령을 막고 실제 실행이 거절되면 돌려준다. 인벤토리 등으로 일시정지한 동안 명령과 정지 타이머도 멈춘다.
- 수동 확인 공간은 `res://tests/fixtures/harness_actions_test_space.tscn`, 자동 검사는 `res://tests/smoke/harness_actions_smoke.tscn`이다. 실행 방법은 [프로젝트 컨텍스트](project_context.md#하네스-분석비상-방전-수동-테스트)에 기록했다.

Godot 4.7.1 프로젝트 초기화, 메인·새 수동 장면 초기화와 스모크 장면 11개가 통과했다. 새 검사에서는 분석 버튼과 포커스가 있는 상태의 실제 `Q` 입력, 공동 충전 소모, 빈 충전의 기본 분석, 반복·범위 이탈·삭제·중첩 요청·실행 거절, 정지 중 이동·일시정지, 탈출·재진입 양쪽 복귀를 확인했다.

충전량·비용·정지 시간은 조정 가능한 시제품 값이다. 물품 감정·가격 공개, 모듈 성장·내구도·충전 복구, 피해·최종 인공지능·저장, 통합 맵과 사람의 재미·제품 수치 판단은 이번 범위에 포함하지 않았다. 구현·자동 검증을 마치고 사용자 승인 후 `8d3884e`로 커밋했다.

## DEV-0111 — G1 통합 플레이테스트와 수치 검토

`res://tests/fixtures/g1_integration_playtest_space.tscn`에 GDD 15-2의 입구·저가 통로·위험 통로·우회로와 기존 기능을 조립했다. 가방 2칸·3칸 중 하나를 선택한 뒤 실제 이동·조사·회수·교체·하네스 대응·귀환을 직접 확인할 수 있다. 시작 전 이동·상호작용을 막고 종료 뒤에는 배우·입력·측정값을 멈춘다.

시험 공간이 공개 상태·사건을 관찰해 탐험 시간과 가방 확인 시간을 분리하고, 적재·충전·추적·귀환 기록을 화면과 로그에 남긴다. 결과의 기록 복사와 조건 재선택은 검사 편의 기능이며 제품 저장·성장·메타 순환에 연결하지 않는다.

`res://tests/smoke/g1_integration_playtest_smoke.tscn`은 순간 이동 없이 실제 이동 입력으로 네 경로를 검증한다. 분석·안정화·방전 뒤 미확인 물품과 함께 귀환하고, 같은 우회 경로에서 2칸·3칸의 차이를 비교하며, 붕괴 범위 잔류 시 회수품 손실도 확인한다. 기존 스모크 검사를 합친 12개가 통과했다. 회수 결과의 긴 설명이 다른 패널을 밀어내던 표시 문제도 함께 수정했다.

[G1 검토 기록](g1_playtest_review.md)에 실행 방법, 원본 자동 자료, GDD 15-2·20-1 대조와 사람 질문을 모았다. `--fixed-fps 60` 자동 입력의 짧은 시간은 사람의 런 시간으로 해석하지 않으며, 무게 감속이 없는 상태에서 최종 감속률을 확정하지 않는다.

통합 장면·자동 검증·검토 자료 준비를 마쳤으며, 사용자 승인으로 `eabc705`를 커밋했다. 커밋 후 추가 검사에서 발견한 분석 버튼 클릭 후 Tab 가방 열기 오류는 사용자 지시에 따라 보완했다. 가방 열기·닫기를 UI 포커스 이동보다 먼저 처리하고, 실제 클릭과 논리·물리 키 코드가 있는 Tab 이벤트로 재현 검사에서 수정 전 실패·수정 후 통과를 확인했다. 재현 검사를 포함한 스모크 장면 12개가 통과했으며 이번 검사 기록·입력 보완은 검증 결과 검토 후 사용자가 커밋을 승인했다.

입력 보완은 사용자 승인으로 `a1aae2f`에 커밋했다. 이후 화면·자유 이동 검토에서 미확인 물품의 긴 설명 잘림과 선택의 부담을 평가하기 어려운 시험 조건을 확인했다. 충전 사용·짐 버리기 없이 주요 회수품 2개를 가져오는 경로가 있었으며, 무료 통과 자체를 오류로 판단하지는 않았다.

2026-09-06 사용자 지시에 따라 가방에 선택한 물품의 상세 표시와 긴 설명 스크롤을 추가했다. 통합 시험 장면의 붕괴 경고를 1.2초, 골렘 의심을 0.7초, 추적 속도를 210으로 조정하고 추적·붕괴 경고 시간을 측정한다. 같은 경로에서 충전 없이 통과하는 경우와 안정화·방전을 사용하는 경우를 추가해 현재 자동 실행 기록은 여섯 개다. 두 경우 모두 같은 회수품 2개를 가져오지만 충전 2회를 사용하면 추적 시간이 7.28초에서 1.38초로 줄었다. 시험 조건만 조정했으며 제품 수치나 피해 규칙은 확정하지 않았다.

상세 정보의 비노출·스크롤·선택 변경 검사와 같은 경로의 대응 비교를 포함해 스모크 장면 12개가 통과했다. 1280×800 화면과 기본 1152×648 배치도 확인했다. 이번 가독성·시험 조건 보완은 검증 결과 검토 후 사용자가 커밋을 승인했다.

**정합성 검사: 지금**이며 실제 사람 평가는 미수행이다. [G1 검토 기록](g1_playtest_review.md)의 사람 질문, GDD 20-1의 판단·`Q-004`·`Q-005` 처리 및 사용자 진행 지시 전까지 G1 통과와 `DEV-0201` 진입은 보류한다.
