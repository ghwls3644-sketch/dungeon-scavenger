...
- [ ] `DEV-0109 — 고장 난 경비 골렘`: 사전 징후와 제한된 순찰·의심·추적 상태를 가진 시험용 경비 골렘을 추가한다. 영구 처치, 전투 피해와 최종 인공지능 수치는 추가하지 않는다.
...
현재 다음 개발 작업은 `DEV-0109 — 고장 난 경비 골렘`이다. `DEV-0108` 결과를 사용자가 검토하고 커밋을 승인하기 전에는 시작하지 않는다. 정합성 검사는 아직 아니며 다음 검사 시점은 `DEV-0111` 완료 뒤다.
