...
충전량·비용·정지 시간은 조정 가능한 시제품 값이다. 물품 감정·가격 공개, 모듈 성장·내구도·충전 복구, 피해·최종 인공지능·저장, 통합 맵과 사람의 재미·제품 수치 판단은 이번 범위에 포함하지 않았다. 구현·자동 검증을 마쳤으며 검증 결과 검토 후 사용자가 커밋을 승인했다. 다음 작업은 `DEV-0111 — G1 통합 플레이테스트와 수치 검토`다. **정합성 검사: 아직 아님**이며 다음 검사는 `DEV-0111` 완료 뒤다.
