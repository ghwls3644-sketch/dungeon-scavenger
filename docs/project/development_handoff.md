...
현재 `DEV-0107` 완료 뒤 G1 정합성 검사 시점이다. 단계 1 티켓의 개별 완료 조건은 통과했으나 GDD 15-2의 별도 조사·미확인 물품·고장 난 경비 골렘·하네스 분석과 비상 방전은 미구현이고, GDD 20-1의 주관적 재미·안도감과 `Q-004`·`Q-005`는 사람 플레이테스트가 필요하다. 계획상 다음 티켓은 `DEV-0201 — Boot와 임시 타이틀`이지만, 사용자가 검사 결과를 검토하고 진행 방향을 지시하기 전에는 시작하지 않는다.
