...
`DEV-0107` 플레이테스트 장면은 적재 선택을 재현하기 위해 슬롯 2개, 부담 2.5, 과적 4.0과 짧은 시험 공간을 사용한다. 이 값은 자동 검사 자료일 뿐 목표 런 시간이나 제품 슬롯·무게 단계·감속률의 근거로 확정하지 않는다. 현재 G1 정합성 검사에서 사람 플레이테스트 결과와 함께 `Q-004`·`Q-005`를 검토해야 하며, 결정 전까지 두 질문은 `provisional`로 유지한다.
