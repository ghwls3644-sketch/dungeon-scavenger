...
| `검토 중` | `DEV-0111` 통합 장면·자료 완료 후 | 2026-09-06 기능 연결과 자동 검사 12개 일치. GDD 20-1 사람 판단과 `Q-004`·`Q-005`는 대기. 사람 검토와 정합성 결과 확인 및 사용자 진행 지시 뒤 `DEV-0201` |
...
**정합성 검사: 지금** — 기능 연결은 GDD 15-2와 일치한다. 통합 장면·검토 자료의 검증 결과를 검토한 뒤 사용자가 커밋을 승인했으며 실제 사람 검토는 미수행이다. GDD 20-1과 `Q-004`·`Q-005`의 남은 판단을 처리하고 사용자 진행 지시를 받기 전까지 G1 통과와 `DEV-0201` 진입을 보류한다. 새 제품 수치·저장 규칙·세계관 변경은 없다.
