...
| `지금` | `DEV-0107` 완료 후 | 단계 1 티켓과 자동 핵심 루프는 통과. GDD 15-2 미구현 범위, 사람 플레이테스트와 `Q-004`·`Q-005` 검토 결과에 따라 `DEV-0201` 또는 보완 작업 결정 |
...
`DEV-0101`~`DEV-0107`의 개별 티켓 완료 조건과 자동화 가능한 GDD 20-1 구조는 통과했다. 다만 GDD 15-2가 G1 범위로 적은 별도 조사, 미확인 물품, 고장 난 경비 골렘, 하네스 분석·비상 방전은 단계 1 티켓에 없어 미구현이며, “더 들어갈지 고민했는가”, “비전투 해결이 만족스러운가”, “귀환이 안도감을 주는가”는 사람 플레이테스트가 필요하다. `Q-004`와 `Q-005`도 시험 장면 값만으로 확정하지 않는다. **정합성 검사: 지금**이며, 사용자 지시 전에는 계획상 다음 티켓 `DEV-0201`이나 보완 작업을 시작하지 않는다.
