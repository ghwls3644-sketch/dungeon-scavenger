...
`DOC-0601` 사람 검토에서 현재 규칙을 그대로 유지하고, 동결 해제 후 첫 GDD 개정에서 3-4의 옛 표현을 현재 규칙에 맞게 고치기로 승인했다. 이번 티켓과 `DOC-0601`에서는 [`GDD.md`](../GDD.md)를 직접 수정하지 않았다.

`DOC-0603`에서 GDD를 활성 제품·시스템 개요로 전환하면서 3-4의 문장을 `범람 위험이 사라진다`로 정정해 `DEC-004`와 일치시켰다. `게임 기획 파일/`의 원본 GDD는 수정하지 않아 기록된 해시를 보존했다.
...
Entity Index와 Timeline의 직접 충돌을 해소했고 `DOC-0506` 링크·누락 자동 검사와 `DOC-0601` 사람 검토도 마쳤다. 두 문서는 미정 이름·연도·기간을 새로 채우지 않는 조건으로 `DOC-0602`의 `provisional` 전환 후보가 되며, 이번 티켓에서는 아직 `draft`를 유지한다.

## 후속 작업

1. `DOC-0602`에서 승인된 문서 상태 전환과 Archive 경고를 적용했다.
2. `DOC-0603`에서 GDD 3-4의 옛 범람 표현을 `DEC-004`의 현재 기준과 맞췄다.
...
`DOC-0601` 사람 검토 결과까지 기록했으며, 이 보고서는 이관 검토 이력으로 `draft`를 유지한다.
