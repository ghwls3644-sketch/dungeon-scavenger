...
| 현재 — `DEV-0107` 완료 후 | `DEV-0101`~`DEV-0107` 결과, GDD 15-2·20-1, G1 재검토 결정과 `Q-004`·`Q-005`가 서로 맞는지 확인 | 검사 결과를 검토하고 사용자가 진행을 지시한 뒤 `DEV-0201` 또는 보완 작업 |
...
현재 `DEV-0107` 완료 체크포인트에 도달했다. `DEV-0101`~`DEV-0107`의 문서화된 티켓 완료 조건과 자동화 가능한 GDD 20-1 항목은 통과했지만, GDD 15-2의 별도 조사·미확인 물품·고장 난 경비 골렘·하네스 분석과 비상 방전은 이 단계 티켓 범위에 포함되지 않아 아직 구현되지 않았다. 재미·안도감 같은 사람 판단과 `Q-004`·`Q-005`도 실제 플레이테스트 전에는 확정할 수 없다. **정합성 검사: 지금**이며, 사용자가 검사 결과를 검토하고 진행 방향을 지시하기 전에는 `DEV-0201`이나 보완 작업을 시작하지 않는다.
