...
검토 관문 G-M2를 통과하고 문서 분할 단계 `M3 — 1차 구조 이관`의 Story 티켓 `DOC-0201`~`DOC-0207`, Design 티켓 `DOC-0301`~`DOC-0304`, Project 티켓 `DOC-0401`~`DOC-0403`을 완료했다. 원본 제목 103개와 구조화 요소 32개의 `planned` 추적 행은 모두 해소되어 G-M3 통과 조건을 충족했으며 사용자 승인을 기다린다. 승인 후 다음 단계는 `M4 — 세계관과 구현 규칙 분리`의 티켓 범위를 정하는 것이다. 신규 문서는 최종 전환 전까지 `draft`다.
