...
검토 관문 G-M2를 통과하고 문서 분할 단계 `M3 — 1차 구조 이관`의 Story 티켓 `DOC-0201`~`DOC-0207`을 완료했다. G-M3 예비 점검에서 Design·Project 대상 21개가 아직 `planned`임을 확인했으므로 관문은 통과시키지 않았다. 다음 작업은 `DOC-0301` 정보 전달 규칙 추출이며, 신규 문서는 최종 전환 전까지 `draft`다.
