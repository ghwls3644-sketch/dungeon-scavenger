...
- G1 통합 플레이 방법과 검토 기록: [`project/g1_playtest_review.md`](project/g1_playtest_review.md)
...
| 현재 검토 — `DEV-0111` 통합 장면·자료 완료 후 | 2026-09-06 가방 상세 표시·시험 조건 보완 완료. 같은 경로의 충전 대응 전후 비교와 무료 생환 확인, 자동 검사 12개 통과. GDD 20-1 사람 판단과 `Q-004`·`Q-005`는 대기 | 사람 검토와 정합성 결과를 확인하고 사용자가 진행을 지시한 뒤 `DEV-0201` |
...
`DEV-0109`에서 발소리·긁힌 흔적을 순찰 중 선행 단서로 보여 주고, 플레이어를 감지하면 탐지음과 의심 단계를 거쳐 추적하는 고장 난 경비 골렘을 추가했다. 시야를 잃으면 마지막 위치를 수색한 뒤 순찰로 돌아가며, 외부 소음과 경보가 같은 공개 상태 전환을 요청할 수 있다. 영구 처치, 기본 공격·피해와 최종 이동·탐지 수치는 추가하지 않았다. Godot 4.7.1 자동 검사에서 순찰 이동, 경고 선행, 의심·추적·수색·순찰 복귀와 소음·경보 요청을 확인했다.

`DEV-0109-R1`에서 사용자 검토로 확인한 보호 물품 버리기, 인벤토리 Tab 입력, 조사 위험 정보 유지, 골렘의 감지·소음 처리, 회수 결과 고정과 탐험 종료 재진입 문제를 보완했다. 기존 스모크 장면에 재현 조건과 정상 경로를 추가해 Godot 4.7.1에서 10개 모두 통과했다. 상세 범위와 결과는 [`개발 인계의 보완 티켓`](project/development_handoff.md#dev-0109-r1--g1-사전-코드-검토-보완)에 있다.

`DEV-0109-R1`은 사용자 승인 후 `4f09ae6`으로 커밋했다.

`DEV-0110`에서 가까운 잔해·골렘의 기본 상태를 무료로 분석하고, 정밀 분석 버튼과 `Q` 안정화·비상 방전이 같은 충전을 쓰도록 했다. 비상 방전은 골렘을 잠시 멈추며 정지 중 반복 사용으로 시간을 늘릴 수 없다. 복귀 시 플레이어가 아직 탐지 범위에 있으면 경고 뒤 추적하고, 벗어났으면 마지막 위치 수색 뒤 순찰한다. 상태가 바뀌거나 대상에서 멀어지면 이전 정밀 분석 표시를 지운다. Godot 4.7.1에서 새 검사를 포함한 스모크 장면 11개와 프로젝트·메인·수동 확인 장면 초기화를 통과했다.

`DEV-0110`은 사용자 승인 후 `8d3884e`로 커밋했다.

`DEV-0111`에서 조사·미확인 물품·골렘·하네스 행동과 기존 회수·생환·실패를 한 작은 맵으로 연결했다. 시작 시 가방 2칸·3칸을 고르고, 결과에서 탐험·가방 확인 시간, 적재 선택과 충전 사용 기록을 복사할 수 있다. 자동 검사는 실제 이동 입력으로 위험 대응, 같은 우회 경로의 가방 비교, 붕괴 실패를 검증한다. 새 검사를 포함한 스모크 장면 12개가 통과했고, 긴 회수 결과 설명의 줄바꿈과 화면 겹침도 보완했다.

**정합성 검사: 지금** — 가방 입력 오류 보완은 사용자 승인으로 `a1aae2f`에 커밋했다. 이후 사용자 지시에 따라 선택 물품 상세 표시와 통합 맵의 시험 조건을 보완했다. 같은 경로에서 충전 대응의 경고·추적 감소와 무료 생환을 확인했으며 자동 검사 12개가 통과했다. [G1 검토 기록](project/g1_playtest_review.md)에 근거를 남겼고 이번 보완은 검증 결과 검토 후 사용자가 커밋을 승인했다. 실제 사람 평가와 `Q-004`·`Q-005`가 남아 있어 G1 통과 및 `DEV-0201` 진입은 보류한다.
