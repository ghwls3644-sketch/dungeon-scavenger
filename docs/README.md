...
검토 관문 G-M2를 통과하고 문서 분할 단계 `M3 — 1차 구조 이관`의 Story 티켓 `DOC-0201`~`DOC-0207`과 Design 티켓 `DOC-0301`~`DOC-0302`를 완료했다. G-M3 예비 점검에서 확인한 나머지 Design·Project 이관을 계속하며, 다음 작업은 `DOC-0303` 아이템 구현 규칙 추출이다. 신규 문서는 최종 전환 전까지 `draft`다.
