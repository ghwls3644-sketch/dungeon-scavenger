...
func _render() -> void:
	...
	if _result.get_unidentified_item_count() > 0:
		_summary.text += " | 미확인 물품 %d개" % _result.get_unidentified_item_count()

	_note.text = "판매·비용 정산 전 예상값이며, 현재 회수품만 포함하고 미확인 물품의 숨겨진 가치는 합계에서 제외합니다."
